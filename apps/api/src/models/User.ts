...
export const createUser = async (newUser: {email: string; password: string }): Promise<PlatformUser> => {
  ...
  try{
    ...
    const created = await db.get<PlatformUser>('SELECT * FROM users WHERE id = ?', result.lastID);
    await db.close();
    return created!;
  } catch (err: any) {
